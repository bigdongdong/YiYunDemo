...
//        this.setAnimationStyle(R.style.pop_alpha_animation);
    ...
    /**
     * 黑科技，通过context拿contentView
     */
    public void showCenteral(){
        View parent = null;
        try{
            parent = ((ViewGroup) context.findViewById(android.R.id.content)).getChildAt(0);
        }catch(Exception e){}
        if(parent == null)return;
        this.showCenteral(parent);
    }
    ...
    public void showAtLocation(View parent, int gravity, int x, int y) {
        if(context != null && !context.isFinishing() && !context.isDestroyed() && !isShowing()){
