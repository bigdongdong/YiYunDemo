...
public abstract class HoverItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private final String TAG = "HoverItemDecorationTAG";
    ...
    private Path getOnDrawOverPath(View parent ,Integer disance){
        ...
        Path p = new Path();
        p.moveTo(parent.getPaddingStart() ,0 - (mHoverHeight - disance));
        p.rLineTo(parent.getWidth() - parent.getPaddingStart() - parent.getPaddingEnd(),0);
        p.rLineTo(0,mHoverHeight);
        p.rLineTo(-parent.getWidth() + parent.getPaddingStart() + parent.getPaddingEnd(),0);
        p.close();
        return p;
        ...
        if(adapter != null){
            ...
            for (int i = 0; i < count; i++) {
                ...
                if(isHover(position)){
                    ...
                    drawHover(c,position,new Rect(child.getLeft(),child.getTop() - mHoverHeight,child.getRight(),child.getTop()));
                    c.restore();
                }
            }
        }
