...
public class MM_CustomEditText2 extends LinearLayout {
    final int dp = 35 ;
    Context context ;
    EditText editText ;
    ImageView canvasIV ;
    ...
    int bitmapH,bitmapW ;
    Canvas canvas ;
    Path firstPath,vPath,hPath;

    public MM_CustomEditText2(Context context) {
        this(context,null);
    ...
    void initView(){
        LayoutInflater.from(context).inflate(R.layout.mm_custom_edittext2,this);
        editText= this.findViewById(R.id.edittext);
        canvasIV= this.findViewById(R.id.canvasIV);

        bitmapH = dp2px(context,dp);
        bitmapW = bitmapH*2;
        bitmap = Bitmap.createBitmap(bitmapW,bitmapH,Bitmap.Config.ALPHA_8);
        canvas = new Canvas();
        canvas.setBitmap(bitmap);
        int end = (int)((double)(bitmapH*(0.7)));
        firstPath = getLine(0,0,0,bitmapH);
        vPath = getLine(bitmapH,0,bitmapH,bitmapH);
        hPath = getLine(bitmapH,end,bitmapW,end);
        canvas.drawPath(firstPath,getPaint(2));
        canvas.drawPath(vPath,getPaint(1));
        canvas.drawPath(hPath,getPaint(2));

        canvasIV.setBackground(new BitmapDrawable(bitmap));
    }

    public MM_CustomEditText2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
